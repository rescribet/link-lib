...
describe("LinkedRenderStore", () => {
    describe("#findSubject", () => {
        ...
        const testData = [
            ...
            [ex("2"), schema.name, ...bill, rdfFactory.defaultGraph()],
            rdfFactory.quad(ex("2"), schema.author, ex("3")),
            rdfFactory.quad(ex("2"), schema.author, ex("4")),

            rdfFactory.quad(ex("3"), rdf.type, schema.Book),
            [ex("3"), schema.name, ...bookTitle, rdfFactory.defaultGraph()],
            [ex("3"), schema.name, ...alternativeTitle, rdfFactory.defaultGraph()],
        ...
        it("resolves an empty path to nothing", () => {
            const answer = store.lrs.findSubject(ex("1"), [], rdfFactory.literal(ex("2")));
